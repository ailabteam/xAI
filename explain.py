...
def create_predict_function(model):
    def predict_fn(x):
        ...
        # --- THAY ĐỔI ---: Đảm bảo đầu ra là mảng 1D
...
def generate_shap_explanations(model, X_test):
    ...
    # Ở đây, shap_values chắc chắn là một mảng numpy 2D vì predict_fn trả về 1D
    
    # 1. & 2. Global Plots (giữ nguyên)
    ...
    if len(attack_index) > 0:
        ...
        single_feature_sample_pd = test_samples.iloc[idx, :]
        single_feature_sample_np = test_samples.iloc[idx, :].values # Chuyển sang numpy
        
        # --- THÊM BƯỚC DEBUG ---
        print("\n--- Debug Info for Force Plot ---")
        print(f"Type of expected_value: {type(expected_value)}, Value: {expected_value}")
        print(f"Type of single_shap_value: {type(single_shap_value)}, Shape: {single_shap_value.shape}")
        print(f"Type of single_feature_sample_np: {type(single_feature_sample_np)}, Shape: {single_feature_sample_np.shape}")
        # -------------------------
        ...
        try:
            force_plot = shap.force_plot(
                expected_value, 
                single_shap_value,
                single_feature_sample_pd, # Vẫn dùng pandas để có tên feature
                matplotlib=True, 
                show=False
            )
            plt.savefig('shap_force_plot_attack_dpi600.png', dpi=600, bbox_inches='tight')
            plt.close()
            print("Saved shap_force_plot_attack_dpi600.png")
        except Exception as e:
            print(f"An error occurred during force_plot generation: {e}")
